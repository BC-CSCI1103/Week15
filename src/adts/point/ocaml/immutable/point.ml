 ...
 * date: January 2020
 *
 * An implementation for a type of points on the 2D plane.
*)
type t = { x : float
         ; y : float
         }
...
let distance p = sqrt((getX p) ** 2.0 +. (getY p) ** 2.0)
...
(* Forcing equal to be consistent with compareTo by fiat.
*)
let equal p1 p2 = (compare p1 p2) = 0

(* To move an immutable point we create a new one.
*)
let move point dx dy =
  make ((getX point) +. dx) ((getY point) +. dy)
